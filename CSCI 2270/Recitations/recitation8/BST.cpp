  ...
  else
  {
    ...
    else
    {
      ///Replace with Minimum from right subtree
        Node* right_child = currNode -> right; 
        if(right_child -> left == NULL){
            right_child -> left = currNode -> left; 
            currNode = right_child; 
        }
        else
        {
            Node* parent_left_min = right_child; 
            Node*left_min = right_child; 
            while(left_min -> left != NULL){
                parent_left_min = left_min; 
                left_min = left_min -> left; 
            }

            parent_left_min -> left = left_min -> right; 
            left_min -> right = currNode -> right; 
            left_min -> left = currNode -> left; 
            currNode = left_min; 
        }
